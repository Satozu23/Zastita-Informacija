...
namespace ZastitaProjekat
{
    public static class Tiger
    {
        ...
        {
            TigerContext ctx = new TigerContext();
            return ctx.Hash(data);
        }

        private class TigerContext
        {
            private ulong A, B, C;
            private ulong AA, BB, CC;
            private ulong[] X = new ulong[8];
            ...
            public byte[] Hash(byte[] array)
            {
                ...
                int numOfFinalBytes = numOfBytes % 64;
                ...
                Buffer.BlockCopy(BitConverter.GetBytes(A), 0, result, 0, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(B), 0, result, 8, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(C), 0, result, 16, 8);
            ...
            private void HashFinalMessageBlock(byte[] finalBytes, int numOfFinalBytes, int messageSizeWithoutPadding)
            {
                if (numOfFinalBytes < 56)
                {
                    ...
                    finalBytes = AppendBytes(finalBytes, padding);

                    ulong msgSizeBits = (ulong)messageSizeWithoutPadding * 8;
                    finalBytes = AppendBytes(finalBytes, BitConverter.GetBytes(msgSizeBits));
                ...
                else
                {
                    ...
                    finalBytes = AppendBytes(finalBytes, paddingMsg1);
                    HashMessageBlock(finalBytes);

                    byte[] paddingMsg2 = new byte[56];
                    ulong msgSizeBits = (ulong)messageSizeWithoutPadding * 8;
                    byte[] finalBlock = AppendBytes(paddingMsg2, BitConverter.GetBytes(msgSizeBits));

                    HashMessageBlock(finalBlock);
                }
            }

            private void HashMessageBlock(byte[] messageBlock)
            {
                for (int i = 0; i < 8; i++)
                {
                    X[i] = BitConverter.ToUInt64(messageBlock, i * 8);
                }
            ...
            private byte[] AppendBytes(byte[] baseArray, byte[] toAppend)
            {
                byte[] newArray = new byte[baseArray.Length + toAppend.Length];
                Buffer.BlockCopy(baseArray, 0, newArray, 0, baseArray.Length);
                Buffer.BlockCopy(toAppend, 0, newArray, baseArray.Length, toAppend.Length);
                return newArray;
            }
        }
